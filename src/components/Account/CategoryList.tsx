...
export default function CategoryList({
  ...
  const { showConfirmModal, onClickClose, onClickConfirm } = useConfirmModal();
              ...
              <div className="flex gap-x-2 text-center">
                <div>
                  <div className="text-green-600">
                    {formatCurrency(
                      categoryTotals?.[category.id]?.incoming || 0
                    )}
                  </div>
                  Income
                </div>
                <div>
                  <div className="text-red-600">
                    {formatCurrency(
                      categoryTotals?.[category.id]?.outgoing || 0
                    )}
                  </div>
                  <div>Expense</div>
                </div>
                <div>
                  <div
                    className={`${
                      (categoryTotals?.[category.id]?.net || 0) >= 0
                        ? "text-green-600"
                        : "text-red-600"
                    }`}
                  >
                    {formatCurrency(categoryTotals?.[category.id]?.net || 0)}
                  </div>
                  <div>Net</div>
                </div>
                <div>
                  <div className="text-gray-600">
                    {categoryTotals?.[category.id]?.count || 0}
                  </div>
                  <div>Count</div>
                </div>
              </div>
